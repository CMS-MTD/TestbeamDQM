...
def main():
    ...
    #271
    #runNumList = [str(x) for x in range(34332, 34350 + 1)] #lecroy
    #272
    runNumList = [str(x) for x in range(34359, 34368 + 1)] #lecroy
    runNumList = [str(x) for x in range(34434, 34445 + 1)] #lecroy
    ...
    photek=7
    beamXRange = "100,-3.0,3.0"
    beamYRange = "100,8.0,14.0"
    ...
    # wider x range
    sel=goodtrack+"&&x_dut[0]>-0.4&&x_dut[0]<0.6&&y_dut[0]>10&&y_dut[0]<11&&amp[7]>100"
    t.Draw(dist+":x_dut[0]>>h(100,-0.4,0.6,7,-0.5,6.5",sel,"COLZ")
    h = getattr(ROOT,"h")
    ...
    for i,ch in enumerate(channels):
        ...
        hist = ROOT.TH2F(hname,";x [mm];amp [mV]", 100,-0.7,1, 100,0,300) 
